var selectSiteModalLinks,selectSiteLastModalInTab;!function($){$(function(){$(".permissions-table input:checkbox").change(function(){$(this).parents("td").toggleClass("active",this.checked)}).change(),$(".permissions-table tr:even").addClass("evenrow"),$(".permissions-table :checked").parents("td").addClass("defaultSelected"),$(".permissions-table .permissionDescription").hover(function(e){$(this).parents("tr").children("td").toggleClass("rowHover","mouseenter"===e.type)}),$(".permissions-table th").hover(function(event){var col=$(this).prevAll().size();$("."+col).add(this).toggleClass("rowHover","mouseenter"===event.type)}),
$(".permissions-table th#permission").hover(function(event){$(".permissions-table td.checkboxCell").toggleClass("rowHover","mouseenter"===event.type)}),$(".permissions-table th#permission a").click(function(e){$(".permissions-table .checkGrid input").prop("checked",0===$(".checkGrid :checked").length).change(),e.preventDefault()}),$(".permissions-table .permissionDescription a").click(function(e){var anyChecked=$(this).parents("tr").find("input:checked").not("[disabled]").length>0;$(this).parents("tr").find("input:checkbox").not("[disabled]").prop("checked",!anyChecked).change(),e.preventDefault()}),
$(".permissions-table th.role a").click(function(e){var col=$(this).parent("th").prevAll().size(),anyChecked=$(".permissions-table ."+col+" input:checked").not("[disabled]").length>0;$(".permissions-table ."+col+" input").not("[disabled]").prop("checked",!anyChecked).change(),e.preventDefault()}),$("#clearall").click(function(e){$(".permissions-table input").not("[disabled]").prop("checked",!1).change(),e.preventDefault()})})}($PBJQ),function($){if(("object"==typeof qtip||"object"==typeof moment||"object"==typeof portal)&&"undefined"!=typeof moment){moment.locale(portal.locale),portal.bullhorn=$("#Mrphs-bullhorn")
;var formatDate=function(instant){return moment.unix(instant.epochSecond).format("L LT")};portal.wrapNoAlertsString=function(noAlertsString){return'<div id="portal-bullhorn-no-alerts">'+noAlertsString+"</div>"},portal.clearBullhornAlert=function(id,noAlerts){$.get("/direct/portal/clearBullhornAlert",{id:id}).done(function(){var alertDiv=$("#portal-bullhorn-alert-"+id),bunch=alertDiv.closest(".portal-bullhorn-bunch"),removeBunch=1===bunch.find(".portal-bullhorn-alert").length,empty=1===$(".portal-bullhorn-alert").length;alertDiv.remove(),empty&&$("#portal-bullhorn-alerts").html(portal.wrapNoAlertsString(noAlerts)),removeBunch&&bunch.remove()
;var count=$(".portal-bullhorn-alert").length;portal.setBullhornCounter(count)})},portal.clearAllBullhornAlerts=function(noAlerts){$.ajax({url:"/direct/portal/clearAllBullhornAlerts",cache:!1}).done(function(){$("#portal-bullhorn-alerts").html(portal.wrapNoAlertsString(noAlerts)),portal.setBullhornCounter(0)})},portal.acceptFriendRequest=function(requestorId,friendId,alertId,noAlertsMessage){confirmFriendRequest(friendId,requestorId),this.clearBullhornAlert(alertId,noAlertsMessage)},portal.ignoreFriendRequest=function(ignorerId,friendId,alertId,noAlertsMessage){ignoreFriendRequest(friendId,ignorerId),
this.clearBullhornAlert(alertId,noAlertsMessage)};var createBunches=function(allAlerts,prefix){const alerts=allAlerts.filter(a=>a.event.startsWith(prefix));var map=new Map;if(alerts.length>0){let startDate=alerts[0].eventDate.epochSecond;map.set(startDate,[]),alerts.forEach(aa=>{let thisDate=aa.eventDate.epochSecond;thisDate<startDate+600?map.get(startDate).push(aa):(startDate=thisDate,map.set(startDate,[aa]))})}return map.forEach(list=>list.sort((a,b)=>b.eventDate.epochSecond-a.eventDate.epochSecond)),map},getBunchMarkup=function(bunch,i18n){var faClass="fa-bullhorn",messageTemplate=i18n.announcement,social=!1
;"assignments"===bunch.type?faClass="fa-file-text":"commons"===bunch.type?(faClass="fa-bank",messageTemplate=i18n.academicCommentCreated):"lessonbuilder"===bunch.type?(faClass="fa-file-text-o",messageTemplate=i18n.academicLessonBuilderCommentCreate):"profile"===bunch.type&&(faClass="icon-sakai--sakai-profile2",social=!0);formatDate({epochSecond:bunch.latest});return markup=function(tool,startDate,faClass,i18n){const formattedStartDate=formatDate({epochSecond:startDate});var toolName="Announcements"
;return"assignments"===tool?toolName="Assignments":"commons"===tool?toolName="Commons":"lessonbuilder"===tool?toolName="Lessons":"profile"===tool&&(toolName="Social Alerts"),
...
}(bunch.type,bunch.latest,faClass,i18n),bunch.alerts.forEach(alert=>{
if("asn.new.assignment"===alert.event||"asn.revise.access"===alert.event?messageTemplate=i18n.assignmentCreated:"asn.grade.submission"===alert.event?messageTemplate=i18n.assignmentSubmissionGraded:"profile.friend.request"===alert.event?messageTemplate=i18n.friendRequest:"profile.friend.confirm"===alert.event?messageTemplate=i18n.friendConfirm:"profile.message.sent"===alert.event?messageTemplate=i18n.message:"profile.status.update"===alert.event?messageTemplate=i18n.statusUpdate:"profile.wall.item.new"===alert.event?messageTemplate=i18n.wallPost:"profile.wall.item.comment.new"===alert.event&&(messageTemplate=i18n.postComment),
social)var message=messageTemplate.replace("{0}",alert.fromDisplayName);else message=messageTemplate.replace("{0}",alert.title).replace("{1}",alert.siteTitle);markup+=function(alert,message,faClass,i18n,social){const formattedDate=formatDate(alert.eventDate)
;var header=`<div id="portal-bullhorn-alert-${alert.id}" class="portal-bullhorn-alert">`,footer=`\n          <div class="portal-bullhorn-time">${formattedDate}</div>\n        </div>\n        <div class="portal-bullhorn-clear">\n          <a href="javascript:;" onclick="portal.clearBullhornAlert('${alert.id}','${i18n.noAlerts}');" title="${i18n.clear}">\n            <i class="fa fa-times" aria-hidden="true"></i>\n          </a>\n        </div>\n      </div>`
...
}(alert,message,faClass,i18n,social)}),markup+="</div></div></div>",markup};$(function(){portal.bullhorn.qtip({suppress:!1,position:{adjust:{scroll:!1},my:"top right",at:"bottom left",target:portal.socialBullhorn},show:{event:"click",delay:0,solo:portal.socialBullhorn},style:{classes:"portal-bullhorns"},hide:{event:"click unfocus"},content:{text:function(event,api){return $.ajax({url:"/direct/portal/bullhornAlerts.json",dataType:"json",cache:!1}).then(function(data){if(data.message&&"NO_ALERTS"===data.message)return portal.wrapNoAlertsString(data.i18n.noAlerts);var markup='<div id="portal-bullhorn-alerts" class="accordion">',allBunches=[]
;return createBunches(data.alerts,"annc").forEach(alerts=>allBunches.push({type:"announcements",alerts:alerts})),createBunches(data.alerts,"asn").forEach(alerts=>allBunches.push({type:"assignments",alerts:alerts})),createBunches(data.alerts,"commons").forEach(alerts=>allBunches.push({type:"commons",alerts:alerts})),createBunches(data.alerts,"lessonbuilder").forEach(alerts=>allBunches.push({type:"lessonbuilder",alerts:alerts})),createBunches(data.alerts,"profile").forEach(alerts=>allBunches.push({type:"profile",alerts:alerts})),allBunches.forEach(b=>{b.latest=b.alerts.reduce((acc,a)=>a.eventDate.epochSecond>acc?a.eventDate.epochSecond:acc,0)
}),allBunches.sort((a,b)=>b.latest-a.latest),allBunches.forEach(b=>{markup+=getBunchMarkup(b,data.i18n)}),markup+=`\n                  <div id="portal-bullhorn-clear-all">\n                    <a href="javascript:;" onclick="portal.clearAllBullhornAlerts('${data.i18n.noAlerts}');">${data.i18n.clearAll}</a>\n                  </div>\n                </div>\n              `},function(xhr,status,error){api.set("content.text",status+": "+error)})}},events:{visible:function(event,api){var firstBunch=document.querySelector("#bullhorn-alerts button");firstBunch&&firstBunch.focus()}}})}),portal.setBullhornCounter=function(count){
var horn=$("#Mrphs-bullhorn");horn.find("#bullhorn-counter").remove(),horn.append('<span id="bullhorn-counter" class="bullhorn-counter-red">'+count+"</span>")};var updateCounts=function(){$.ajax({url:"/direct/portal/bullhornAlertCount.json",cache:!1,data:{auto:!0}}).done(function(data){data>0?portal.setBullhornCounter(data):portal.bullhorn.find("#bullhorn-counter").remove()}).fail(function(xhr,status,error){console&&console.log("Failed to get the bullhorn counts. Status: "+status),console&&console.log("FAILED ERROR: "+error),clearInterval(portal.bullhornCountIntervalId)})}
;portal.loggedIn&&portal.bullhorns&&portal.bullhorns.enabled&&(updateCounts(),portal.bullhornCountIntervalId=setInterval(updateCounts,portal.bullhorns.pollInterval))}}($PBJQ),$PBJQ("#footerAppPresence").on("click",function(){$PBJQ("#presenceArea").toggleClass("is-hidden")}),function($){if(portal.loggedIn){portal.connectionManager=portal.connectionManager||{};var connectionTemplate=Handlebars.templates["connection-manager-connection"],searchResultTemplate=Handlebars.templates["connection-manager-searchresult"],indexedCurrentConnections={},indexedPendingConnections={},countPending=function(){return Object.keys(indexedPendingConnections).length
},indexedSearchResults={},lastSearchResults={};$(function(){portal.i18n.loadProperties({resourceClass:"org.sakaiproject.portal.api.PortalService",resourceBundle:"connection-manager",namespace:"connection-manager",callback:function(){portal.i18n.loadProperties({resourceClass:"org.sakaiproject.portal.api.PortalService",resourceBundle:"profile-popup",namespace:"connection-manager"})}})});var currentTotal=0,currentState="connections",shown=0,pendingTabBaseHtml="";portal.connectionManager.show=function(options){var connectionManager=$("#connection-manager");connectionManager.modal({width:320})
;var connectionsView=$("#connection-manager-connectionsview"),searchResultsView=$("#connection-manager-searchresultsview"),searchResultsCount=$("#connection-manager-searchresultsview-count"),currentTab=$("#connection-manager-navbar-current > span > a"),pendingTab=$("#connection-manager-navbar-pending > span > a"),updateSearchResultsCount=function(count){if(0===count)searchResultsCount.html(portal.i18n.tr("connection-manager","connection_manager_no_results"));else{var translateOptions={count:count,criteria:portal.connectionManager.searchCriteria
},countMessage=count>1?portal.i18n.tr("connection-manager","connection_manager_results_count",translateOptions):portal.i18n.tr("connection-manager","connection_manager_result_count",translateOptions);searchResultsCount.html(countMessage)}},showCurrentTab=function(){pendingConnectionsWrapper.hide(),currentConnectionsWrapper.show(),connectionsView.show(),searchResultsView.hide(),currentTab.parent().addClass("current"),pendingTab.parent().removeClass("current")},showPendingTab=function(){currentConnectionsWrapper.hide(),pendingConnectionsWrapper.show(),connectionsView.show(),searchResultsView.hide(),pendingTab.parent().addClass("current"),
currentTab.parent().removeClass("current")},addPendingAndShowTab=function(friendId,displayName){var countBefore=countPending();$("#connection-manager-connection-"+friendId).remove(),1===searchResults.children().length&&searchResultsWrapper.hide();var connection=indexedSearchResults[friendId];connection.connected=!1,connection.hideConnect=!0,connection.outgoing=!0,indexedPendingConnections[friendId]=connection;var markup=connectionTemplate(connection);0===countBefore&&(pendingConnectionsDiv.show().html(""),noPendingConnectionsDiv.hide()),pendingConnectionsDiv.append(markup),updatePendingTabText(),
("connections"===currentState||"searchresults"==currentState&&0===moreSearchResults.children().length)&&showPendingTab(),$("#connection-manager-cancel-button-"+friendId).click(cancelHandler)};currentTab.click(function(e){showCurrentTab()}),pendingTab.click(function(e){showPendingTab()});var searchResultsWrapper=$("#connection-manager-connectionsview-searchresults-wrapper"),searchResults=$("#connection-manager-connectionsview-searchresults"),moreSearchResults=$("#connection-manager-searchresultsview-results");connectionManager.click(function(e){if("connection-manager-connectionsview-searchbox"!==e.target.id){
var wrapperRect=searchResultsWrapper[0].getBoundingClientRect();searchBox[0].getBoundingClientRect();(e.pageX<wrapperRect.left||e.pageY<wrapperRect.top||e.pageX>wrapperRect.left+wrapperRect.width||e.pageY>wrapperRect.top+wrapperRect.height)&&searchResultsWrapper.hide()}})
...
search(this.value,!1)}),searchBox.keydown(function(e){13==e.which&&this.value.length>=4&&$("#connection-manager-connectionsview-searchresults-more").click()}),moreSearchBox.keyup(function(e){search(this.value,!0)}),options&&"pending"===options.state&&showPendingTab()},$("#Mrphs-userNav__submenuitem--connections").click(portal.connectionManager.show)}}($PBJQ),$PBJQ(document).ready(function(){var footerDetails=$PBJQ("#Mrphs-footer--details__info");1===footerDetails.length&&(footerDetails.popover({html:!0,content:function(){return $PBJQ("#Mrphs-footer--details__panelTemplate").html()}}),footerDetails.click(function(e){e.preventDefault()})),
updateFooterTime=function(){if(1==$PBJQ("#preferredTime").length)var preferredTzDisplay=$PBJQ("#preferredTime").data("preferredtzdisplay"),preferredLocalOffset=new Date(parseInt($PBJQ("#preferredTime").data("preferredserverdateandgmtoffset"))).getTime()-(new Date).getTime();var serverTzDisplay=$PBJQ("#serverTime").data("servertzdisplay"),serverLocalOffset=new Date(parseInt($PBJQ("#serverTime").data("serverdateandgmtoffset"))).getTime()-(new Date).getTime();return function(){var dateString=new Date((new Date).getTime()+serverLocalOffset).toUTCString().replace(/GMT/,serverTzDisplay).replace(/UTC/,serverTzDisplay)
;if($PBJQ("#serverTime").text(dateString),1==$PBJQ("#preferredTime").length){dateString=new Date((new Date).getTime()+preferredLocalOffset).toUTCString().replace(/GMT/,preferredTzDisplay).replace(/UTC/,preferredTzDisplay);$PBJQ("#preferredTime").text(dateString)}setTimeout("updateFooterTime()",1e3)}}(),1==$PBJQ("#serverTime").length&&updateFooterTime()}),$PBJQ,portal.loggedIn&&(window.sakai=window.sakai||{},window.sakai.translations=window.sakai.translations||{},portal.i18n.loadProperties=function(options){if(options.namespace){var defaults={resourceClass:"org.sakaiproject.i18n.InternationalizedMessages",
resourceBundle:"org.sakaiproject.".concat(options.namespace).concat(".bundle.Messages"),namespace:options.namespace};(options=Object.assign(defaults,options)).debug&&(console.debug("resourceClass: "+options.resourceClass),console.debug("resourceBundle: "+options.resourceBundle),console.debug("namespace: "+options.namespace)),window.sakai.translations[options.namespace]=window.sakai.translations[options.namespace]||{};var storageKey=portal.locale+options.resourceBundle;null!==sessionStorage.getItem(storageKey)?(options.debug&&console.debug("Returning "+storageKey+" from sessions storage ..."),
window.sakai.translations[options.namespace]=JSON.parse(sessionStorage[storageKey]),options.callback&&options.callback()):(options.debug&&console.debug(storageKey+" not in sessions storage. Pulling from webservice ..."),$PBJQ.ajax({url:"/sakai-ws/rest/i18n/getI18nProperties"+portal.portalCDNQuery,cache:!0,contentType:"application/json",data:{locale:portal.locale,resourceclass:options.resourceClass,resourcebundle:options.resourceBundle}}).done(function(data,textStatus,jqXHR){data.split("\n").forEach(function(pair){var keyValue=pair.split("=");2==keyValue.length&&(window.sakai.translations[options.namespace][keyValue[0]]=keyValue[1]),
options.debug&&(console.debug("Updated translations: "),console.debug(window.sakai.translations[options.namespace]))}),sessionStorage[storageKey]=JSON.stringify(window.sakai.translations[options.namespace]),options.callback&&options.callback()}).fail(function(jqXHR,textStatus,errorThrown){console.error(errorThrown)}))}else console.error("You must supply at least a namespace. Doing nothing ...")},portal.i18n.tr=function(namespace,key,options){if(namespace&&key){var ret=window.sakai.translations[namespace][key];if(!ret)return console.warn(namespace+"#key "+key+" not found. Returning key ..."),key
;if(null!=options)for(var prop in options)ret=ret.replace("{"+prop+"}",options[prop]);return ret}console.error("You must supply a namespace and a key. Doing nothing.")},Handlebars.registerHelper("tr",function(namespace,key,options){return new Handlebars.SafeString(portal.i18n.tr(namespace,key,options.hash))})),$PBJQ(document).ready(function(){var _defaults={dragModeStartMsg:"drag start, Use U and D keys to move the item up and down",dragModeEndMsg:"Drag end",newPositionMsg:"Moved to position %s"};$PBJQ.widget("sakai.keyboardSortable",$PBJQ.ui.sortable,{options:{keyboardHandle:!1},_create:function(){this._super(),
this.options.keyboardHandle=this.options.keyboardHandle||this.options.items,this._dragging=!1,this.element.on("keydown",this.options.keyboardHandle,$PBJQ.proxy(this._keydown,this)),this.liveRegion=$PBJQ("<div class='ui-helper-hidden-accessible'></div>").liveregion().appendTo(document.body),this.element.find(this.options.keyboardHandle).attr({"aria-grabbed":"false","data-at-shortcutkeys":'{"u":"drag item up","d":"drag item down"}'}).blur($PBJQ.proxy(function(event){var $target=$PBJQ(event.target);this._isDragging()&&this._toggleDragMode($target,this._closestSortableNode($target))},this)).click($PBJQ.proxy(function(event){
if(event.clientX<=0||void 0===event.clientX){var $target=$PBJQ(event.target);this._toggleDragMode($target,this._closestSortableNode($target))}},this))},_keydown:function(event){if(-1!==$PBJQ.inArray(event.which,[27,38,40,68,85])&&!$PBJQ(event.target).is(":input")){event.preventDefault();var $target=$PBJQ(event.target),$sortableNode=this._closestSortableNode($target);switch(event.which){case 27:this._isDragging()&&this._toggleDragMode($target,$sortableNode);break;case 38:case 85:(event.ctrlKey||this._isDragging()||85===event.which)&&this._moveBackward($sortableNode,$target);break;case 40:case 68:
...
var autoFavoritesEnabled=!0,initialFavoritesList=void 0,favoritesList=[],maxFavoriteEntries=$PBJQ("#max-favorite-entries").text().trim(),favoritesLoaded=!1,container=$PBJQ("#selectSite"),favoritesPane=$PBJQ("#otherSitesCategorWrap"),organizePane=$PBJQ("#organizeFavorites"),topNavPane=$PBJQ("#topnav"),initialTopBarSiteDisplayOrder=$PBJQ(".Mrphs-sitesNav__favbtn",topNavPane).map(function(){return $PBJQ(this).data("site-id")}).toArray(),itemsBySiteId={};$PBJQ(".site-favorite-btn",favoritesPane).each(function(i,e){itemsBySiteId[$PBJQ(e).attr("data-site-id")]=$PBJQ(e).parent()});var button_states={favorite:{
markup:'<i class="site-favorite-icon site-favorite"></i>'},nonfavorite:{markup:'<i class="site-favorite-icon site-nonfavorite"></i>'},myworkspace:{markup:'<i class="site-favorite-icon site-workspace site-favorite"></i>'}},getUserFavorites=function(callback){$PBJQ.ajax({url:"/portal/favorites/list",method:"GET",dataType:"json",success:function(data){autoFavoritesEnabled=data.autoFavoritesEnabled,favoritesList=data.favoriteSiteIds.filter(function(e,i){return""!=e}),null==initialFavoritesList&&(initialFavoritesList=favoritesList),callback(favoritesList)}})},setButton=function(btn,state){var entry=button_states[state]
;$PBJQ(btn).data("favorite-state",state),"favorite"===state?$PBJQ(btn).attr("title",$PBJQ("#removeFromFavoritesText").text().replace("[site]",$PBJQ(btn).parent().find("span.fullTitle").text())):"nonfavorite"===state?$PBJQ(btn).attr("title",$PBJQ("#addToFavoritesText").text().replace("[site]",$PBJQ(btn).parent().find("span.fullTitle").text())):$PBJQ(btn).attr("title",null),$PBJQ(btn).empty().append($PBJQ(entry.markup))},renderFavoriteCount=function(){var favoriteCount=$PBJQ(".fav-sites-entry .site-favorite",favoritesPane).length;$PBJQ(".favoriteCount",container).text("("+favoriteCount+")"),
favoriteCount>maxFavoriteEntries?$PBJQ(".favoriteCountAndWarning").addClass("maxFavoritesReached"):$PBJQ(".favoriteCountAndWarning").removeClass("maxFavoritesReached")},setAllOrNoneStarStates=function(){$PBJQ(".favorites-select-all-none",favoritesPane).each(function(idx,selectAllNone){var termContainer=$PBJQ(selectAllNone).closest(".fav-sites-term"),siteCount=termContainer.find(".fav-sites-entry:not(.my-workspace)").length,favoritedSiteCount=termContainer.find(".fav-sites-entry .site-favorite").length;0==siteCount?$PBJQ(selectAllNone).hide():(favoritedSiteCount==siteCount?($PBJQ(selectAllNone).data("favorite-state","favorite"),
$PBJQ(selectAllNone).html(button_states.favorite.markup)):($PBJQ(selectAllNone).data("favorite-state","nonfavorite"),$PBJQ(selectAllNone).html(button_states.nonfavorite.markup)),$PBJQ(selectAllNone).show())})},renderFavorites=function(favorites){$PBJQ(".site-favorite-btn",favoritesPane).each(function(idx,btn){var buttonSiteId=$PBJQ(btn).attr("data-site-id");$PBJQ(btn).closest(".my-workspace").length>0?setButton(btn,"myworkspace"):$PBJQ.inArray(buttonSiteId,favorites)>=0?setButton(btn,"favorite"):setButton(btn,"nonfavorite")}),$PBJQ(".favorites-help-text").hide(),
autoFavoritesEnabled?$PBJQ(".favorites-help-text.autofavorite-enabled").show():$PBJQ(".favorites-help-text.autofavorite-disabled").show(),setAllOrNoneStarStates(),renderFavoriteCount(),favoritesLoaded=!0},toggleTopNavFavorite=function(event){event.preventDefault();var thisFavButton=$PBJQ(event.target),newFavId=thisFavButton.data("site-id");getUserFavorites(function(list){var favs=list,ind=favs.indexOf(newFavId);if(-1===ind){var favIdIndex=initialTopBarSiteDisplayOrder.indexOf(newFavId);-1!==favIdIndex?favs.splice(favIdIndex,0,newFavId):favs.push(newFavId)}else favs.splice(ind,1);thisFavButton.toggleClass("non-fav"),
thisFavButton.toggleClass("fav");var thisFavButtonForJS=thisFavButton[0];"true"===thisFavButtonForJS.getAttribute("aria-checked")?thisFavButtonForJS.setAttribute("aria-checked","false"):thisFavButtonForJS.setAttribute("aria-checked","true"),function(favs,onError){onError||(onError=function(err){});var newState={favoriteSiteIds:favs,autoFavoritesEnabled:autoFavoritesEnabled};$PBJQ.ajax({url:"/portal/favorites/update",method:"POST",data:{userFavorites:JSON.stringify(newState)},error:onError}),favoritesList=favs}(favs)})};$PBJQ(".Mrphs-sitesNav__favbtn").each(function(i,e){return $PBJQ(e).click(toggleTopNavFavorite)})
;var loadFromServer=function(attempt){syncInProgress&&(favoritesLoaded=!1,$PBJQ(".site-favorite-btn",favoritesPane).empty(),$PBJQ(".favorites-select-all-none",favoritesPane).empty()),attempt||(attempt=0),syncInProgress&&attempt<100?setTimeout(function(){loadFromServer(attempt+1)},50):getUserFavorites(renderFavorites)},syncInProgress=!1,nextToSync=[],runNextServerUpdate=function(onError){for(var newState;nextToSync.length>0;)newState=nextToSync.shift();newState?$PBJQ.ajax({url:"/portal/favorites/update",method:"POST",dataType:"json",data:{userFavorites:JSON.stringify(newState)},error:onError,complete:runNextServerUpdate}):syncInProgress=!1
},syncWithServer=function(onError){if(favoritesLoaded){onError||(onError=function(){});var newFavorites=$PBJQ(".site-favorite-btn",favoritesPane).has(".site-favorite").map(function(){return $PBJQ(this).attr("data-site-id")}).toArray(),newState={favoriteSiteIds:newFavorites=newFavorites.sort(function(a,b){return-1===favoritesList.indexOf(a)?1:-1===favoritesList.indexOf(b)?-1:favoritesList.indexOf(a)-favoritesList.indexOf(b)}),autoFavoritesEnabled:autoFavoritesEnabled};nextToSync.push(newState),syncInProgress||(syncInProgress=!0,runNextServerUpdate(onError)),favoritesList=newFavorites,function(){if(function(a1,a2){
if(a1.length!=a2.length)return!1;for(var i=0;i<a1.length;i++)if(a1[i]!=a2[i])return!1;return!0}(favoritesList,initialFavoritesList))$PBJQ(".moresites-refresh-notification").remove();else if(!($PBJQ(".moresites-refresh-notification").length>0)){var notification=$PBJQ('<div class="moresites-refresh-notification" />').html($PBJQ("#refreshNotificationText").html());$PBJQ("#loginLinks").prepend(notification),notification.css("top",$PBJQ(".Mrphs-siteHierarchy").offset().top+"px")}}()}},returnElementToOriginalPositionIfPossible=function(siteId){if(initialFavoritesList&&initialFavoritesList.indexOf(siteId)>-1){
for(var idx=initialFavoritesList.indexOf(siteId),placed=!1,neighborIdx=idx-1;neighborIdx>=0;neighborIdx--){var neighbor=initialFavoritesList[neighborIdx],neighborCurrentIndex=favoritesList.indexOf(neighbor);if(neighborCurrentIndex>=0&&neighborCurrentIndex<idx){favoritesList.splice(neighborCurrentIndex+1,0,siteId),placed=!0;break}}placed||favoritesList.splice(idx,0,siteId)}};$PBJQ(favoritesPane).on("click",".site-favorite-btn",function(){var newState,siteId=$PBJQ(this).attr("data-site-id"),originalState=$PBJQ(this).data("favorite-state")
...
$PBJQ("#messageHolder").fadeIn("slow"),$PBJQ("#messageHolder").animate({opacity:1},2e3,function(){$PBJQ(that).val(oldVal),that.focus(),that.select()}),$PBJQ("#messageHolder").fadeOut("slow"),$PBJQ(this).parents(".reorder-element").addClass("orderable-selected"),null}var inputs=$PBJQ('input[id^="index"]');$PBJQ("#undo-last").fadeIn("slow"),$PBJQ("#undo-last-inact").hide(),$PBJQ("#undo-all").fadeIn("slow"),$PBJQ("#undo-all-inact").hide(),
"1"===newVal?$PBJQ($PBJQ(this).parents(".reorder-element")).insertBefore($PBJQ(this).parents(".reorder-element").siblings(".reorder-element").children("span").children("input[value="+newVal+"]").parents(".reorder-element")):newVal==inputs.length?$PBJQ($PBJQ(this).parents(".reorder-element")).insertAfter($PBJQ(this).parents(".reorder-element").siblings(".reorder-element").children("span").children("input[value="+newVal+"]").parents(".reorder-element")):newVal>oldVal?$PBJQ($PBJQ(this).parents(".reorder-element")).insertAfter($PBJQ(this).parents(".reorder-element").siblings(".reorder-element").children("span").children("input[value="+newVal+"]").parents(".reorder-element")):$PBJQ($PBJQ(this).parents(".reorder-element")).insertBefore($PBJQ(this).parents(".reorder-element").siblings(".reorder-element").children("span").children("input[value="+newVal+"]").parents(".reorder-element")),
registerChange("notfluid",$PBJQ(this).parents(".reorder-element"))}),$PBJQ('[id$="reorder-list"]').keyboardSortable({items:".reorder-element:not(.notsortable)",start:function(event,ui){preserveStatus(ui)},update:function(event,ui){registerChange(event,ui)}})});var registerChange=function(originEvent,movedEl){$PBJQ('[id$="reorder-list"] .reorder-element').size();"notfluid"!==originEvent&&(movedEl=$PBJQ(document.activeElement).closest('[id^="ui-id-"]')),$PBJQ("#lastItemMoved").text($PBJQ(movedEl).attr("id")).trigger("change"),$PBJQ(movedEl).addClass("recentMove");$PBJQ(movedEl.prevAll(".reorder-element").length+1)
...
$PBJQ(movedEl).animate({opacity:1},2e3,function(){$PBJQ(movedEl).removeClass("recentMove")})},preserveStatus=function(item){$PBJQ("#lastMoveArray").text(""),$PBJQ('[id$="reorder-list"] .reorder-element').each(function(n){void 0!==$PBJQ(this).attr("id")&&"undefined_avatar"!==$PBJQ(this).attr("id")&&$PBJQ("#lastMoveArray").append($PBJQ(this).attr("id")+" ")})};function toggleToolsNav(event){event&&event.preventDefault(),$PBJQ("body").toggleClass("toolsNav--displayed"),$PBJQ("body").hasClass("toolsNav--displayed")?createDHTMLMask(toggleToolsNav):removeDHTMLMask()}$PBJQ(document).ready(function(){$PBJQ("#roleSwitchSelect").on("change",function(){
...
$PBJQ(".user-dropdown-overlay").remove(),$PBJQ(document.body).off("keyup",handleKeyUp),$PBJQ("#roleSwitchDropDownToggle").focus())})}():$PBJQ("#roleSwitch").addClass("menu-not-setup")}),$PBJQ(".js-toggle-tools-nav","#skipNav").on("click",toggleToolsNav);var sessionTimeOut,timeoutDialogWarningTime,timeoutLoggedoutUrl,timeoutPortalPath,sessionId="current",timeoutDialogEnabled=!1;$PBJQ(document).ready(function(){portal.loggedIn&&portal.timeoutDialog&&setTimeout("setup_timeout_config();",6e4)});var setup_timeout_config=function(){portal&&portal.timeoutDialog&&(timeoutDialogEnabled=portal.timeoutDialog.enabled,
timeoutDialogWarningTime=portal.timeoutDialog.seconds,timeoutLoggedoutUrl=portal.loggedOutUrl,timeoutPortalPath=portal.portalPath,1==timeoutDialogEnabled&&(poll_session_data(),fetch_timeout_dialog()))},poll_session_data=function(){$PBJQ.ajax({url:"/direct/session/"+sessionId+".json?auto=true&_="+(new Date).getTime(),dataType:"json",success:function(data){if(data.maxInactiveInterval=1e3*data.maxInactiveInterval,data.active&&null!=data.userId&&data.lastAccessedTime+data.maxInactiveInterval>data.currentTime){var remaining=data.lastAccessedTime+data.maxInactiveInterval-data.currentTime
;remaining<1e3*timeoutDialogWarningTime?(min=Math.round(remaining/6e4),show_timeout_alert(min),clearTimeout(sessionTimeOut),sessionTimeOut=setTimeout("poll_session_data()",6e4)):(clearTimeout(sessionTimeOut),sessionTimeOut=setTimeout("poll_session_data()",remaining-1e3*timeoutDialogWarningTime))}else null==data.userId?location.href=timeoutLoggedoutUrl:sessionTimeOut=setTimeout("poll_session_data()",1e4)},error:function(XMLHttpRequest,status,error){}})};function keep_session_alive(){dismiss_session_alert(),$PBJQ.get(timeoutPortalPath)}var timeoutDialogFragment,dismiss_session_alert=function(){removeDHTMLMask(),
$PBJQ("#timeout_alert_body").remove()};function fetch_timeout_dialog(){$PBJQ.ajax({url:"/portal/timeout?auto=true",cache:!0,dataType:"text",success:function(data){timeoutDialogFragment=data},error:function(XMLHttpRequest,status,error){timeoutDialogEnabled=!1}})}function show_timeout_alert(min){if(timeoutDialogEnabled)if($PBJQ("#portalMask").get(0)||(createDHTMLMask(dismiss_session_alert),$PBJQ("#portalMask").css("z-index",1e3)),$PBJQ("#timeout_alert_body").get(0))$PBJQ("#timeout_alert_body span").html(min);else{var dialog=timeoutDialogFragment.replace("{0}",min);$PBJQ("body").append(dialog)}}
function toggleShortUrlOutput(defaultUrl,checkbox,textbox){$PBJQ(checkbox).is(":checked")?$PBJQ.ajax({url:"/direct/url/shorten?path="+encodeURI(defaultUrl),dataType:"text",success:function(shortUrl){$PBJQ("."+textbox).val(shortUrl)}}):$PBJQ("."+textbox).val(defaultUrl)}$PBJQ(document).ready(function(){$PBJQ(".Mrphs-toolTitleNav__link--directurl").click(function(e){var origin=$PBJQ(this).position(),$dialog=$PBJQ(this).siblings(".Mrphs-directUrl");$dialog.toggleClass("active").css({left:origin.left+"px"}),$dialog.attr("aria-expanded","true"),$dialog.find("[tabindex]").first().focus(),e.preventDefault()}),
$modal_container=$PBJQ(".Mrphs-directUrl"),$modal_container.each(function(index){var $invokerTabs=$PBJQ(this).find("[tabindex]");$invokerTabs.first().on("keydown",function(e){if(9===e.keyCode&&e.shiftKey)return $invokerTabs.last().focus(),e.preventDefault(),!1}),$invokerTabs.last().on("keydown",function(e){if(9===e.keyCode&&!e.shiftKey)return $invokerTabs.first().focus(),e.preventDefault(),!1})}),$modal_container.last().on("keypress",function(e){if(9===e.keyCode())return console.log("keypress:: primero"),$modal_container.fist().focus(),e.preventDefault(),!1}),$PBJQ(".Mrphs-directUrl .dropDown_close").on("click keypress",function(e){
var $dialog=$PBJQ(this).parent();$dialog.toggleClass("active"),$dialog.removeAttr("aria-expanded"),$PBJQ('.Mrphs-toolTitleNav__link--directurl[rel="#'+$dialog.attr("id")+'"]').focus(),e.preventDefault()})});var setupSkipNav=function(){$PBJQ("#skipNav a.Mrphs-skipNav__link").click(function(){var target=$PBJQ(this).attr("href");$PBJQ(target).attr("tabindex","-1").focus()})};$PBJQ(document).ready(function(){var lastScrollTop=0;$PBJQ(document).scroll(function(event){var st=$PBJQ(this).scrollTop();st>lastScrollTop&&st>90?($PBJQ(".Mrphs-topHeader").addClass("moving"),$PBJQ(".Mrphs-siteHierarchy").addClass("moving"),
$PBJQ(".Mrphs-toolsNav__title--current-site").addClass("moving"),$PBJQ(".Mrphs-skipNav__menu").addClass("moving"),$PBJQ(".Mrphs-sitesNav__menuitem--myworkspace").addClass("moving")):(st>90||0==st)&&($PBJQ(".Mrphs-topHeader").removeClass("moving"),$PBJQ(".Mrphs-siteHierarchy").removeClass("moving"),$PBJQ(".Mrphs-toolsNav__title--current-site").removeClass("moving"),$PBJQ(".Mrphs-skipNav__menu").removeClass("moving"),$PBJQ(".Mrphs-sitesNav__menuitem--myworkspace").removeClass("moving")),lastScrollTop=st})});var closeAllDropdownMenus=function(){$PBJQ(".Mrphs-sitesNav__menuitem").removeClass("dropdown-is-visible"),
$PBJQ(".Mrphs-sitesNav__menuitem").find(".is-visible").removeClass("is-visible"),$PBJQ(".Mrphs-sitesNav__menuitem").find(".is-clicked").removeClass("is-clicked"),$PBJQ(".sitenav-dropdown-overlay").remove()},buildDropdownMenu=function(container,siteId,callback){
