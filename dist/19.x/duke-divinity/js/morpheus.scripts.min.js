...
hitType:"event",eventCategory:"toolTitleNav",eventAction:"showLessonsPrintView",eventLabel:document.title};ga("send",gaEventData)}),$("#show-pages").on("click",function(e){var gaEventData={hitType:"event",eventCategory:"toolTitleNav",eventAction:"showLessonsPages",eventLabel:document.title};ga("send",gaEventData)}),$("#loginLink1").find(".Mrphs-login-Message").text().startsWith("Return")&&$("body").addClass("become-user");var serverClass=new URL(window.location.href).hostname.replace(/\./g,"-");$("body").addClass(serverClass),$("body").addClass(portal.user.siteRole.toLowerCase())
;var targetNode,collapseToolsAdjustedTop=0,watchForPasystemChange=function(){var pasystemNode=document.getElementsByClassName("pasystem-banner-alerts")[0],pasystemToggleNode=document.getElementsByClassName("pasystem-banner-alert-toggle")[0],config={childList:!0,attributes:!0,subtree:!0,attributeOldValue:!0},pasystemMutationCallback=function(mutationsList,pasystemObserver){for(var mutation of mutationsList)"childList"==mutation.type&&(collapseToolsAdjustedTop=pasystemNode.clientHeight,$("#toolsNav-toggle-li").css({top:collapseToolsAdjustedTop})),
"attributes"==mutation.type&&mutation.target.className.includes("pasystem-banner-alert")&&(collapseToolsAdjustedTop=pasystemNode.clientHeight,$("#toolsNav-toggle-li").css({top:collapseToolsAdjustedTop}))},pasystemObserver=new MutationObserver(pasystemMutationCallback),pasystemToggleObserver=new MutationObserver(pasystemMutationCallback);pasystemObserver.observe(pasystemNode,config),pasystemToggleObserver.observe(pasystemToggleNode,config)};if(targetNode=document.getElementsByClassName("Mrphs-portalBody")[0],new MutationObserver(function(mutationsList,portalObserver){
for(var mutation of mutationsList)"childList"==mutation.type&&"pasystem-banner-alerts"==mutation.target.childNodes[0].className&&(collapseToolsAdjustedTop=document.getElementsByClassName("pasystem-banner-alerts")[0].clientHeight,$("#toolsNav-toggle-li").css({top:collapseToolsAdjustedTop}),watchForPasystemChange(),portalObserver.disconnect())}).observe(targetNode,{childList:!0}),$(window).scroll(function(){$(window).scrollTop()>0?$("#toolsNav-toggle-li").css({top:0}):$("#toolsNav-toggle-li").css({top:collapseToolsAdjustedTop})}),"Sakai : Home : Overview"===document.title){window.setTimeout(function(){
