...
$PBJQ("#loginPortalMask").remove(),$PBJQ("#Mrphs-xlogin-container").remove(),$PBJQ(".Mrphs-portalWrapper").removeClass("blurry")}),e.preventDefault()})}),$PBJQ(".Mrphs-toolTitleNav__link--directurl").on("click",function(e){var label=$PBJQ(this).siblings(".Mrphs-toolTitleNav__link--help-popup")[0].search,gaEventData={hitType:"event",eventCategory:"toolTitleNav",eventAction:"showDirectLink",eventLabel:document.title+" : "+label.substring(6,label.length)};ga("send",gaEventData)}),$PBJQ(".Mrphs-toolTitleNav__link--help-popup").on("click",function(e){var label=$PBJQ(this)[0].search,gaEventData={hitType:"event",eventCategory:"toolTitleNav",
eventAction:"goToHelp",eventLabel:document.title+" : "+label.substring(6,label.length)};ga("send",gaEventData)}),$PBJQ("#print-view").on("click",function(e){var label=$PBJQ(this)[0].search,gaEventData={hitType:"event",eventCategory:"toolTitleNav",eventAction:"lessionsPrintView",eventLabel:document.title+" : "+label.substring(6,label.length)};ga("send",gaEventData)}),$PBJQ("#print-all").on("click",function(e){var label=$PBJQ(this)[0].search,gaEventData={hitType:"event",eventCategory:"toolTitleNav",eventAction:"lessionsPrintAll",eventLabel:document.title+" : "+label.substring(6,label.length)};ga("send",gaEventData)}),
$PBJQ("#show-pages").on("click",function(e){var label=$PBJQ(this)[0].search,gaEventData={hitType:"event",eventCategory:"toolTitleNav",eventAction:"lessionsIndexOfPages",eventLabel:document.title+" : "+label.substring(6,label.length)};ga("send",gaEventData)}),$PBJQ(".Mrphs-sitesNav__dropdown").on("click",function(e){var label=$PBJQ(this).siblings("a.link-container")[0].title,gaEventData={hitType:"event",eventCategory:"sitesNav",eventAction:"openToolDropdown",eventLabel:document.title+" : "+label};ga("send",gaEventData)}),$PBJQ(".Mrphs-hierarchy--toolName").on("click",function(e){var gaEventData={hitType:"event",eventCategory:"hierarchy",
eventAction:"refreshTool",eventLabel:document.title};ga("send",gaEventData)}),$PBJQ(".view-all-sites-btn").on("click",function(e){var gaEventData={hitType:"event",eventCategory:"topHeader",eventAction:"openAllSitesDropdown",eventLabel:document.title};ga("send",gaEventData)}),$PBJQ("#toolsNav-toggle-li").on("click",function(e){var toggleState="undefined";$PBJQ(this).hasClass("min")?toggleState="collapseTools":$PBJQ(this).hasClass("max")&&(toggleState="maximizeTools");var gaEventData={hitType:"event",eventCategory:"toolMenu",eventAction:toggleState,eventLabel:document.title};ga("send",gaEventData)}),$PBJQ("#print-view").on("click",function(e){
var gaEventData={hitType:"event",eventCategory:"toolTitleNav",eventAction:"showLessonsPrintView",eventLabel:document.title};ga("send",gaEventData)}),$PBJQ("#show-pages").on("click",function(e){var gaEventData={hitType:"event",eventCategory:"toolTitleNav",eventAction:"showLessonsPages",eventLabel:document.title};ga("send",gaEventData)});var dukeAddBodyClasses=function(){var bodyClasses=[],serverClass=new URL(window.location.href).hostname.replace(/\./g,"-");bodyClasses.push(`duke-${serverClass}`);var userSiteRole=`duke-role-${portal.user.siteRole.toLowerCase()}`;bodyClasses.push(userSiteRole),
$PBJQ("#loginLink1").find(".Mrphs-login-Message").text().startsWith("Return")&&bodyClasses.push("duke-become-user"),document.getElementsByTagName("body")[0].classList.add(...bodyClasses)};dukeAddBodyClasses();var dukeCollapseToolsAdjustedTop=0,dukeAdjustNodesForPASystem=function(nodes,top){document.querySelectorAll(nodes).forEach(el=>el.style.top=`${top}px`)},dukeWatchForPasystemLoad=function(nodesToMove){var targetNode=document.getElementsByClassName("Mrphs-portalBody")[0];new MutationObserver(function(mutationsList,portalObserver){
for(var mutation of mutationsList)"childList"==mutation.type&&"pasystem-banner-alerts"==mutation.target.childNodes[0].className&&(dukeCollapseToolsAdjustedTop=document.getElementsByClassName("pasystem-banner-alerts")[0].clientHeight,dukeAdjustNodesForPASystem(nodesToMove,dukeCollapseToolsAdjustedTop),dukeWatchForPasystemChange(nodesToMove),portalObserver.disconnect())}).observe(targetNode,{childList:!0})},dukeWatchForPasystemChange=function(nodesToMove){var pasystemNode=document.getElementsByClassName("pasystem-banner-alerts")[0],pasystemToggleNode=document.getElementsByClassName("pasystem-banner-alert-toggle")[0],config={childList:!0,
attributes:!0,subtree:!0,attributeOldValue:!0},pasystemMutationCallback=function(mutationsList,pasystemObserver){for(var mutation of mutationsList)"childList"==mutation.type&&(dukeCollapseToolsAdjustedTop=pasystemNode.clientHeight,dukeAdjustNodesForPASystem(nodesToMove,dukeCollapseToolsAdjustedTop)),"attributes"==mutation.type&&mutation.target.className.includes("pasystem-banner-alert")&&(dukeCollapseToolsAdjustedTop=pasystemNode.clientHeight,dukeAdjustNodesForPASystem(nodesToMove,dukeCollapseToolsAdjustedTop))
},pasystemObserver=new MutationObserver(pasystemMutationCallback),pasystemToggleObserver=new MutationObserver(pasystemMutationCallback);pasystemObserver.observe(pasystemNode,config),pasystemToggleObserver.observe(pasystemToggleNode,config)},dukeWatchForPASystemOnScroll=function(nodes){window.addEventListener("scroll",event=>{window.scrollY>0?dukeAdjustNodesForPASystem(nodes,0):dukeAdjustNodesForPASystem(nodes,dukeCollapseToolsAdjustedTop)})};if(dukeWatchForPasystemLoad("#toolsNav-toggle-li"),dukeWatchForPASystemOnScroll("#toolsNav-toggle-li"),"Sakai : Home : Overview"===document.title){window.setTimeout(function(){
let motdCards=document.getElementsByClassName("Mrphs-toolBody--sakai-motd")[0].getElementsByTagName("iframe")[0].contentWindow.document.getElementsByClassName("textPanel"),motdArr=[].slice.call(motdCards),tallestMOTD=Math.max.apply(Math,motdArr.map(function(card){return card.clientHeight}));for(let i=0;i<motdArr.length;i++)motdCards[i].style.height=`${tallestMOTD}px`,void 0!==motdCards[i].getElementsByTagName("a")[0]&&(motdCards[i].getElementsByTagName("a")[0].style.cssText="bottom: 16px")},1e3)}
if((document.querySelector('#msgForum .itemNav .button[title=" < Previous Topic"]')||document.querySelector('#msgForum .itemNav .button[title=" Next Topic >"]'))&&document.getElementsByClassName("topicBloc topicBlocLone specialLink")&&window.innerWidth>1030){let forumDescHeight=49+document.getElementsByClassName("topicBloc topicBlocLone specialLink")[0].clientHeight;document.querySelector("#msgForum .itemNav").style.cssText=`top:${forumDescHeight}px`}if("null"!==document.getElementById("footer-links")){let buildTimeHtml="<dt>Skin Version:</dt><dd>Thu, 02 Jul 2020 21:03:34 UTC</dd>"
;document.querySelector("#Mrphs-footer--details__panel dl").insertAdjacentHTML("beforeend",buildTimeHtml)}!function($){if(console.log("duke-dku/_customization.js loaded"),document.querySelector(".Mrphs-sakai-samigo #takeAssessmentForm .honor-container")){console.log("issue 193 executing"),
...
}if(document.querySelector(".Mrphs-sakai-assignment-grades #honor-pledge-agreement")){console.log("issue 193 executing");let asnHonorPledgeLabel=document.querySelector(".Mrphs-sakai-assignment-grades #honor-pledge-agreement")
;asnHonorPledgeLabel.innerHTML='\n\t\t\t<div class="container"> \n\t\t\t\t<div class="col-xs-12">\n\t\t\t\t\t<h4>Duke Kunshan Community Standard</h4>\n\t\t\t\t\t<p>Duke Kunshan University (DKU) is a community comprised of individuals from diverse cultures and backgrounds. We are dedicated to scholarship, leadership, and service and to the principles of honesty, fairness, respect, and accountability. Members of this community commit to reflecting upon and upholding these principles in all academic and non-academic endeavors and to protecting and promoting a culture of integrity and trust.</p>\n\t\t\t\t\t<p>To uphold the DKU Community Standard</p>\n\t\t\t\t\t<ul>\n\t\t\t\t\t\t<li>I will hold myself to the highest standards for honesty, integrity, fairness, and responsibility in my academic and non-academic endeavors</li>\n\t\t\t\t\t\t<li>I will respect other cultures and embrace all forms of diversity</li>\n\t\t\t\t\t\t<li>I will uphold the standards if they are compromised.</li>\n\t\t\t\t\t</ul>\n\t\t\t\t</div>\n\t\t\t</div>',
asnHonorPledgeLabel.insertAdjacentHTML("afterend","<p><strong>Honor Pledge: I will neither give nor receive aid on this assessment.</strong></p>"),asnHonorPledgeLabel.style.display="block"}}($PBJQ);