function footerDetailsPanelEscHandler(e){27===e.keyCode&&toggleFooterDetailsPanel(e)}function toggleFooterDetailsPanel(e){e.preventDefault(),$PBJQ("#Mrphs-footer--details__panel").toggleClass("Mrphs-footer--details__panel-show"),$PBJQ("#Mrphs-footer--details__panel").hasClass("Mrphs-footer--details__panel-show")?($PBJQ(document).on("keyup",footerDetailsPanelEscHandler),$PBJQ("#Mrphs-footer--details__close").focus()):($PBJQ(document).off("keyup",footerDetailsPanelEscHandler),$PBJQ("#Mrphs-footer--details__info").focus())}var selectSiteModalLinks,selectSiteLastModalInTab;!function($){$(function(){
$(".permissions-table input:checkbox").change(function(){$(this).parents("td").toggleClass("active",this.checked)}).change(),$(".permissions-table tr:even").addClass("evenrow"),$(".permissions-table :checked").parents("td").addClass("defaultSelected"),$(".permissions-table .permissionDescription").hover(function(e){$(this).parents("tr").children("td").toggleClass("rowHover","mouseenter"===e.type)}),$(".permissions-table th").hover(function(event){var col=$(this).prevAll().size();$("."+col).add(this).toggleClass("rowHover","mouseenter"===event.type)}),$(".permissions-table th#permission").hover(function(event){
$(".permissions-table td.checkboxCell").toggleClass("rowHover","mouseenter"===event.type)}),$(".permissions-table th#permission a").click(function(e){$(".permissions-table .checkGrid input").prop("checked",0===$(".checkGrid :checked").length).change(),e.preventDefault()}),$(".permissions-table .permissionDescription a").click(function(e){var anyChecked=$(this).parents("tr").find("input:checked").not("[disabled]").length>0;$(this).parents("tr").find("input:checkbox").not("[disabled]").prop("checked",!anyChecked).change(),e.preventDefault()}),$(".permissions-table th.role a").click(function(e){
var col=$(this).parent("th").prevAll().size(),anyChecked=$(".permissions-table ."+col+" input:checked").not("[disabled]").length>0;$(".permissions-table ."+col+" input").not("[disabled]").prop("checked",!anyChecked).change(),e.preventDefault()}),$("#clearall").click(function(e){$(".permissions-table input").not("[disabled]").prop("checked",!1).change(),e.preventDefault()})})}($PBJQ),function($){if(("object"==typeof qtip||"object"==typeof moment||"object"==typeof portal)&&"undefined"!=typeof moment){moment.locale(portal.locale),portal.socialBullhorn=$PBJQ("#Mrphs-social-bullhorn"),portal.academicBullhorn=$PBJQ("#Mrphs-academic-bullhorn")
;var formatDate=function(instant){return moment.unix(instant.epochSecond).format("L LT")};portal.wrapNoAlertsString=function(noAlertsString){return'<div class="portal-bullhorn-no-alerts">'+noAlertsString+"</div>"},portal.clearBullhornAlert=function(type,id,noAlerts){$PBJQ.get("/direct/portal/clearBullhornAlert",{id:id}).done(function(){var alertDiv=$PBJQ("#portal-bullhorn-alert-"+id),empty=1===$PBJQ(".portal-bullhorn-"+type+"-alert").length;alertDiv.remove(),empty&&$PBJQ(".portal-bullhorn-"+type+"-alerts").html(portal.wrapNoAlertsString(noAlerts));var count=$PBJQ(".portal-bullhorn-"+type+"-alert").length;portal.setCounter(type,count)})},
portal.clearAllAlerts=function(type,noAlerts){$PBJQ.ajax({url:"/direct/portal/clearAll"+type+"Alerts",cache:!1}).done(function(){$PBJQ(".portal-bullhorn-"+type.toLowerCase()+"-alerts").html(portal.wrapNoAlertsString(noAlerts)),portal.setCounter(type.toLowerCase(),0)})},portal.acceptFriendRequest=function(requestorId,friendId,alertId,noAlertsMessage){confirmFriendRequest(friendId,requestorId),this.clearBullhornAlert(portal.SOCIAL,alertId,noAlertsMessage)},portal.ignoreFriendRequest=function(ignorerId,friendId,alertId,noAlertsMessage){ignoreFriendRequest(friendId,ignorerId),this.clearBullhornAlert(portal.SOCIAL,alertId,noAlertsMessage)},
